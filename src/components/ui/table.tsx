...
function TableHeader({ className, ...props }: React.ComponentProps<'thead'>) {
  return (
    <thead
      ...
      className={cn('bg-muted/25 [&_tr]:border-b [&_tr]:border-border/60', className)}
      {...props}
    />
  );
}
...
function TableRow({ className, ...props }: React.ComponentProps<'tr'>) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        'border-b border-border/60 transition-colors hover:bg-muted/35 data-[state=selected]:bg-muted/60',
        className
      )}
      {...props}
    />
